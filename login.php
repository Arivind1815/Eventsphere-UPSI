...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($matric_id_err) && empty($password_err)) {
        // Prepare a select statement - only select users with student role
        $sql = "SELECT id, name, matric_id, email, password, role FROM users WHERE matric_id = ? AND role = 'student'";
        ...
        if ($stmt = $conn->prepare($sql)) {
            ...
            if ($stmt->execute()) {
                ...
                // Check if matric ID exists with student role, if yes then verify password
                if ($stmt->num_rows == 1) {
                    ...
                    if ($stmt->fetch()) {
                        ...
                        if ($password === $db_password) {
                            ...
                            $login_err = "Invalid Matric ID or password.";
                        }
                    }
                } else {
                    // Matric ID doesn't exist or user is not a student
                    $login_err = "Invalid student credentials or you don't have student access.";
                }
            } else {
...
<head>
    ...
    <title>Student Login - EventSphere@UPSI</title>
        ...
        <div class="form-container">
            ...
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                <div class="form-group">
                    ...
                    <input type="text" name="matric_id" class="form-control <?php echo (!empty($matric_id_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $matric_id; ?>" placeholder="Enter your Student Matric ID">
                ...
                <div class="form-group">
                    <button type="submit" class="btn btn-primary">Login to Student Portal</button>
                </div>
                <div class="login-link">
                    <p>New student? <a href="register.php">Register here</a></p>
                </div>
                <div class="admin-notice">
                    <p>Faculty or Admin? <a href="admin_login.php">Go to Admin Login</a></p>
                </div>
            </form>
        </div>
    </div>
